...
import static com.campusland.views.ViewMain.serviceCliente;
...
public class RepositoryFacturaMysqlImpl implements RepositoryFactura {
    ...
    private Factura creaFactura(ResultSet rs) throws SQLException {
        ...
        final int id_descuento = rs.getInt("id_descuento");
    ...
    public List<Factura> obtenerFacturasEnRango(LocalDateTime fechaInicio, LocalDateTime fechaFin) {
        ...
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM factura WHERE fecha BETWEEN ? AND ?")) {
            stmt.setObject(1, Timestamp.valueOf(fechaInicio));
            stmt.setObject(2, Timestamp.valueOf(fechaFin));
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                Logger.getLogger(RepositoryFacturaMysqlImpl.class.getName()).log(Level.SEVERE, null, ex);
            }
        } catch (SQLException throwables) {
            throw new RuntimeException("Error al obtener facturas en rango", throwables);
        }

        return facturasEnRango;
    }

     private Factura mapResultSetToFactura(ResultSet rs) throws SQLException, ClienteNullException {
        int numeroFactura = rs.getInt("numeroFactura");
        LocalDateTime fecha = rs.getTimestamp("fecha").toLocalDateTime();
        String documentoCliente = rs.getString("documento_cliente");
        int id_descuento = rs.getInt("id_descuento");
        
        try {
            Cliente cliente = serviceCliente.porDocumento(documentoCliente); // Utilizamos este método ahora
            return new Factura(id_descuento, numeroFactura, fecha, cliente);
        } catch (ClienteNullException e) {
            // Manejar la excepción según tus necesidades (puedes lanzarla nuevamente o manejarla de otra manera)
            e.printStackTrace();
            return null;
        }
    }
